...
export async function openQrScanner(onResult){
  ...
  const area = document.getElementById('qrArea'); area.replaceChildren(video);
  const stream = await navigator.mediaDevices.getUserMedia({ video:{ facingMode:'environment' }});
  video.srcObject = stream;
  const canvas = document.createElement('canvas'); const ctx = canvas.getContext('2d');
  let detector = null, jsqrLoaded = false;
    ...
    if (video.readyState>=2){
      ...
      try{
        if (detector){
          ...
          if (!jsqrLoaded){ await import('https://cdn.jsdelivr.net/npm/jsqr@1.4.0/dist/jsQR.js'); jsqrLoaded=true; }
...
export function makeQr(el, text){
  import('https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js').then(()=>{
    el.innerHTML=''; new window.QRCode(el, { text, width:196, height:196 });
  });
}
